...
class KernelModule(nn.Module):
    ...
    def estimate_fisher_diagonal(self, x: torch.Tensor, score: torch.Tensor, t: float, alpha: float = None) -> torch.Tensor:
        ...
        Args:
            ...
            alpha: ᾱ(t) value; if provided it is used both in computation and cache key
        """
        ...
        # Include ᾱ in the cache key to avoid reusing across very different noise levels
        t_part = f"{t:.6f}"
        ...
        if TRITON_AVAILABLE and self.config.use_triton_kernels and x.is_cuda and x.numel() > 1e6:
            ...
            # default alpha if not provided: assume ᾱ≈1−t (only as fallback)
