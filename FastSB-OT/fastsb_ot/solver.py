...
class FastSBOTSolver(nn.Module):
    """Production-ready Fast Schrödinger Bridge with Optimal Transport Solver.

    Args:
        score_model: Neural network that predicts noise ε(x_t, t)
        noise_schedule: Function mapping t∈[0,1] → ᾱ(t) (cumulative product of (1-β)).
                       ...
                       NOT β(t) or α(t), but ᾱ(t) = Π(1-β_i).
    ...
    def _validate_noise_schedule(self):
        """Validate that noise_schedule returns monotonically decreasing α_bar(t)"""
        ...
        # Check monotonic decrease (α_bar(0) ≥ α_bar(1/32) ≥ ... ≥ α_bar(1))
        non_monotonic = []
        ...
        if non_monotonic:
            raise ValueError(
                f"noise_schedule must return α_bar(t) monotonically decreasing in t∈[0,1]. "
                f"Found non-monotonic segments: {non_monotonic[:3]}... "
                f"α_bar should decrease from ~1 at t=0 (clean) to ~0 at t=1 (noise)."
            )

        # Also check reasonable range
        if not (0.8 <= vals[0] <= 1.0):
            logger.warning(f"α_bar(0) = {vals[0]:.4f} is unusually low. Expected ~1.0 for clean data.")
        if not (0.0 <= vals[-1] <= 0.2):
            logger.warning(f"α_bar(1) = {vals[-1]:.4f} is unusually high. Expected ~0.0 for pure noise.")
    ...
    def compute_score_cached(self, x: torch.Tensor, t: float, cache_key: Optional[str] = None) -> torch.Tensor:
        ...
        # POLISH: Using clearer variable names (alpha_bar_t instead of alpha_t)
        alpha_bar_t = self._get_cached_noise_schedule(t)

        # Score = -noise / sigma, where sigma = sqrt(1 - alpha_bar)
        sigma_t = math.sqrt(max(1e-8, 1.0 - alpha_bar_t))

        # Apply variance-preserving scaling
        if self.config.variance_preserving_sampling:
            score = -noise_pred / sigma_t
        else:
            # Standard DDPM parameterization
            score = -noise_pred / sigma_t
    ...
    def compute_score_patches_fixed(self, x: torch.Tensor, t: float, run_uid: Optional[int] = None) -> torch.Tensor:
        ...
        with torch.no_grad():
            with self._amp_ctx():
                patch_scores = self.score_model(patches, t_tensor)

        # POLISH: Using clearer variable names
        alpha_bar_t = self._get_cached_noise_schedule(t)
        sigma_t = math.sqrt(max(1e-8, 1.0 - alpha_bar_t))
        patch_scores = -patch_scores / sigma_t
    ...
    def ddpm_step_improved(self, x_t: torch.Tensor, noise_pred: torch.Tensor,
        ...
        if self.config.use_learned_variance:
            ...
            if noise_pred.shape[1] != x_t.shape[1] and noise_pred.shape[1] != expected_channels:
                raise ValueError(
                    ...
                    f"(fixed variance) or 2×C={expected_channels} channels ([ε||logvar]). "
        ...
        # Use the actual next sampling time as the "previous" ᾱ to match the chosen discretization
        alpha_bar_prev = alpha_bar_next if t_next > 0 else 1.0

        # Numerically safe β_t
        ...
        # The posterior mean is: μ = √ᾱ_{t-1} * β_t/(1-ᾱ_t) * x0 + √α_t * (1-ᾱ_{t-1})/(1-ᾱ_t) * x_t
        denominator = max(1 - alpha_bar_t, 1e-12)
...
def make_schedule(schedule_type: str = "linear",
    ...
    """Create noise schedule function returning α_bar(t) for t∈[0,1]

    POLISH: Clearer documentation about continuous-time α_bar
    ...
    Returns:
        Function mapping t∈[0,1] to α_bar(t)∈[0,1]

    Note: This returns the continuous-time α_bar(t) = exp(-∫β(s)ds),
    not the discrete product form. For discrete DDPM schedules,
    use your own schedule function.
    """

    def linear_schedule(t: float) -> float:
        """Linear beta schedule → exponential α_bar"""
        # Continuous integral form: α_bar(t) = exp(-∫β(s)ds)
        beta_t = beta_start + t * (beta_end - beta_start)
        # Integral of linear: ∫(a + bs)ds = as + bs²/2
...
def example_usage():
    ...
    # Create noise schedule (α_bar(t) from 1→0)
