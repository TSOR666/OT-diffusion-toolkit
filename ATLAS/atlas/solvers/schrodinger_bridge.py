...
class SchroedingerBridgeSolver:
    ...
    def __init__(
        ...
        # Numerical guard for conjugate gradient denominators
        self._min_curvature = 1e-12
    ...
    def _conjugate_gradient(
        ...
        for i in range(max_iter):
            ...
            min_denom = max(self._min_curvature, torch.finfo(denom.dtype).tiny)
    ...
    def validate_timesteps(self, timesteps: Sequence[float]) -> List[float]:
        ...
        for prev, curr in zip(schedule, schedule[1:]):
            if prev - curr <= min_delta:
                raise ValueError(
                    "Timesteps must be strictly decreasing with a minimum spacing of 1e-6."
                )
        return schedule
