"""Training script for CIFAR-10 (32x32) using the ATLAS preset."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a CIFAR-10 diffusion model using preset defaults."
    )
    parser.add_argument(
        "--data-root",
        default=None,
        metavar="PATH",
        help="Path to the CIFAR-10 dataset root (will be downloaded if missing).",
    )
    parser.add_argument(
        "--checkpoints",
        default=None,
    ...
    parser.add_argument(
        "--preset",
        default="experiment:cifar10",
        metavar="NAME",
        help="Preset configuration to use (default: experiment:cifar10).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.data_root:
        ...
        try:
            ...
            errors.append(f"Cannot create dataset directory: {exc}")
        else:
...
def _print_header(args: argparse.Namespace) -> None:
    ...
    print("=" * 70)
    print("CIFAR-10 32x32 Training")
    print("=" * 70)
    print("This preset is optimized for quick iterations on a small dataset.")
    print("Images will download automatically if CIFAR-10 is not present.")
    print("=" * 70)
    ...
    print("-" * 70)
    print("Typical requirements:")
    print("  - VRAM: ~2-4 GB (mixed precision enabled by default)")
    print("  - Disk: ~1 GB for CIFAR-10 + checkpoints")
    print("=" * 70 + "\n")
...
def main() -> None:
    ...
    print("Next steps: run inference with atlas.examples.cifar10_inference.py")
