...
class FFTOptimalTransport:
    ...
    def _multiscale_sinkhorn_fft(
        ...
        mu_coarse = F.interpolate(
            mu.unsqueeze(0).unsqueeze(0), size=current_shape, mode="bilinear" if len(shape) == 2 else "trilinear"
        ).squeeze(0).squeeze(0)
        nu_coarse = F.interpolate(
            nu.unsqueeze(0).unsqueeze(0), size=current_shape, mode="bilinear" if len(shape) == 2 else "trilinear"
        ).squeeze(0).squeeze(0)
        ...
        for level in range(self.scale_levels, -1, -1):
            ...
            u_upsampled = F.interpolate(
                u_coarse.unsqueeze(0).unsqueeze(0), size=current_shape, mode="bilinear" if len(shape) == 2 else "trilinear"
            ).squeeze(0).squeeze(0)
            v_upsampled = F.interpolate(
                v_coarse.unsqueeze(0).unsqueeze(0), size=current_shape, mode="bilinear" if len(shape) == 2 else "trilinear"
            ).squeeze(0).squeeze(0)

            mu_current = F.interpolate(
                mu.unsqueeze(0).unsqueeze(0), size=current_shape, mode="bilinear" if len(shape) == 2 else "trilinear"
            ).squeeze(0).squeeze(0)
            nu_current = F.interpolate(
                nu.unsqueeze(0).unsqueeze(0), size=current_shape, mode="bilinear" if len(shape) == 2 else "trilinear"
            ).squeeze(0).squeeze(0)
