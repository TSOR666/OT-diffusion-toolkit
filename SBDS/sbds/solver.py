...
SINKHORN_ITERATIONS_NYSTROM = 30  # Iterations for Nyström Sinkhorn
...
class EnhancedScoreBasedSBDiffusionSolver:
    ...
    def _variance_reduced_score(self, x: torch.Tensor, t: float, n_samples: int = 5) -> torch.Tensor:
        ...
        For stochastic score estimates s_i ~ p(s|x,t), the variance-reduced estimator is:
            s_VR = Σ w_i * s_i  where w_i ∝ 1/Var(s_i)
        ...
        weight = weight / weight.mean()  # Normalize weights
    ...
    def _rff_sb_transport(self, x_t: torch.Tensor, t_curr: float, t_next: float, iterations: int = 3) -> Tuple[torch.Tensor, float]:
        ...
        for iter in range(iterations):
            ...
            # Normalize features for numerical stability in Sinkhorn iterations
            # NOTE: This changes the kernel approximation k(x,y) ≈ φ(x)ᵀφ(y)
            # to k_normalized(x,y) ≈ φ̃(x)ᵀφ̃(y) where φ̃ = φ/||φ||
            # Trade-off: Sacrifices perfect kernel approximation for numerical stability
            # in the subsequent Sinkhorn algorithm, preventing overflow in exponentials.
            # Empirically, this works well for diffusion models as the approximation
            # preserves relative distances: ||φ̃(x) - φ̃(y)||² ≈ 2(1 - φ(x)ᵀφ(y)/||φ(x)||||φ(y)||)
    ...
    def _fft_sb_transport(self, x_t: torch.Tensor, t_curr: float, t_next: float, iterations: int = 3) -> Tuple[torch.Tensor, float]:
        ...
        This method uses a Gaussian density surrogate ρ(x) ∝ exp(-x²/2) as an approximation
        ...
        - For intermediate times, the Wasserstein-2 error is bounded by:
          W₂(p_t, ρ) ≤ C·√t(1-t) where C depends on the score function
        ...
        for iter in range(iterations):
            ...
            # For grid-structured data, we expect x_pred to have shape [batch, *spatial_dims, channels]
            # After summing over batch (line 929), u has shape [*spatial_dims, channels]
            # _compute_gradient_on_grid returns one gradient per dimension of u
            if x_pred.dim() <= 2:
                # FFT transport requires explicit spatial structure (at least 3D: batch, spatial, channels)
                # Reject flattened data like [batch, features] early to prevent shape mismatches
                raise ValueError(
                    f"FFT transport requires grid-structured data with explicit spatial dimensions. "
                    f"Got x_pred with shape {x_pred.shape} (dim={x_pred.dim()}). "
                    f"Expected at least 3 dimensions: [batch, *spatial_dims, channels]. "
                    f"For 2D point clouds, use RFF or Nyström transport instead."
                )

            # u has shape [*spatial_dims, channels] after summing over batch dimension
            # So it has (x_pred.dim() - 1) dimensions total
            expected_grad_dims = x_pred.dim() - 1
            ...
            if len(grad_u) != expected_grad_dims:
                raise ValueError(
                    ...
                    f"x_pred shape: {x_pred.shape}, u shape after batch sum: {tuple(x_pred.shape[1:])}, "
            ...
            for i in range(x_pred.size(0)):
                for d, grad in enumerate(grad_u):
                    ...
                    x_pred[i, ..., d] = x_pred[i, ..., d] - FFT_TRANSPORT_STEP_SIZE * grad
    ...
    def _multiscale_sb_transport(self, x_t: torch.Tensor, t_curr: float, t_next: float, iterations: int = 3) -> Tuple[torch.Tensor, float]:
        """
        Multi-scale Schrödinger Bridge transport with proper warm-starting.
    ...
    def _corrector_update(self, x: torch.Tensor, t: float) -> torch.Tensor:
        ...
        for _ in range(self.corrector_steps):
            ...
            step_size = self.corrector_snr * (1 - alpha_t) / (score_norms + 1e-8)
