...
class OptimizedSinkhornKernel:
    ...
    def sinkhorn_log_stabilized(self, x: torch.Tensor, y: torch.Tensor, eps: float, n_iter: int = 10) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _sinkhorn_native(self, x: torch.Tensor, y: torch.Tensor, eps: float, n_iter: int):
        ...
        with torch.amp.autocast(device_type=device_type, enabled=False):
            ...
            if not (torch.isfinite(lu).all() and torch.isfinite(lv).all()):
                logger.warning("Non-finite Sinkhorn duals (eps=%s), using identity fallback - may affect quality", eps)
                u = torch.full((N,), float("nan"), device=self.device, dtype=self.dtype)
                v = torch.full((M,), float("nan"), device=self.device, dtype=self.dtype)
                return u, v

        u = lu.exp().to(self.dtype)
        v = lv.exp().to(self.dtype)

        if not (torch.isfinite(u).all() and torch.isfinite(v).all()):
            logger.warning("Non-finite values after exp in Sinkhorn, using identity fallback - may affect quality")
            u = torch.full((N,), float("nan"), device=self.device, dtype=self.dtype)
            v = torch.full((M,), float("nan"), device=self.device, dtype=self.dtype)
    ...
    def _sinkhorn_blockwise(self, x: torch.Tensor, y: torch.Tensor, eps: float, n_iter: int):
        ...
        u = log_u.exp().to(self.dtype)
        v = log_v.exp().to(self.dtype)
        return u, v
    ...
    def _sinkhorn_triton(self, x: torch.Tensor, y: torch.Tensor, eps: float, n_iter: int):
        ...
        if not (torch.isfinite(u).all() and torch.isfinite(v).all()):
            logger.warning("Non-finite values after exp in Triton Sinkhorn")
            u = torch.full((N,), float("nan"), device=self.device, dtype=self.dtype)
            v = torch.full((M,), float("nan"), device=self.device, dtype=self.dtype)
    ...
    def _sinkhorn_pot(self, x: torch.Tensor, y: torch.Tensor, eps: float, n_iter: int):
        ...
        v_np = log.get("v")
        if u_np is None:
            u_np = np.exp(log.get("logu", np.zeros(x.size(0), dtype=np.float32)))
        if v_np is None:
            v_np = np.exp(log.get("logv", np.zeros(y.size(0), dtype=np.float32)))

        u = torch.from_numpy(u_np.astype(np.float32)).to(self.device, self.dtype)
        v = torch.from_numpy(v_np.astype(np.float32)).to(self.device, self.dtype)
